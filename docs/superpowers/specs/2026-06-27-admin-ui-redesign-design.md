...
관리자가 다크모드를 선호하여 admin은 **웜 다크 테마**로 한다(공개 사이트의 밝은 톤과 별개).

- 표면: 본문 배경 `#1A1814`(웜 다크), 카드 `#23211B`, 사이드바 `#12110E`, 입력 배경 `#1A1814`
- 경계선: `#322F28`(카드), `#3A3730`(입력·버튼 테두리), `#2C2A24`(셸 구분선)
- 텍스트: 주요 `#F3EEE2`(밝은 크림), 보조 `#A89F90`, 힌트 `#7C7466`
- 포인트(골드): `#C9A96E` — 주요 버튼 배경(글자 `#241D10`)·활성 메뉴. 활성 메뉴는 골드 좌측 보더(`2px`) + 은은한 골드 틴트(`rgba(201,169,110,0.16)`, 글자 `#E7C98E`)
- 강조(미공개·미처리): 부드러운 레드 텍스트 `#E0857B`, 배지 배경 `#C0392B`(글자 흰색)
- 폰트: 본문 Noto Sans KR(`--font-noto-sans-kr`), 일부 헤더 Serif(`--font-noto-serif-kr`)

admin은 디자인 핸드오프 대상이 아니므로 자체 스타일 자유 — 헌법[7]의 공개 페이지 보존과 무관. 위 값은 `ui.module.css`/`admin-shell.module.css` 상단에 CSS 변수로 정의해 재사용한다.
